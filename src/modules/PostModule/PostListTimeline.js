...
//   const [commentInputVisible, setCommentInputVisible] = useState({});
      ...
      try {
        ...
        const enrichedPosts = await Promise.all(
          response.data.map(async (post) => {
            ...
            //sortinf for comments
            const sortedComments=commentResponse.data.sort((a,b)=> new Date(b.commentTimeStamp)- new Date(a.commentTimeStamp) );
        ...
        .sort((a,b)=> new Date(b.postTimeStamp)- new Date(a.postTimeStamp) );
  ...
  const handleDeletePost = () => {
    axios
      .delete(`http://localhost:5000/post-service/posts/${username}/${currentPostId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
      })
